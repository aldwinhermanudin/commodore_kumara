...
void USBHID::PrintEndpointDescriptor(const USB_ENDPOINT_DESCRIPTOR* ep_ptr) {
        ...
        printf("\r\nLength:\t\t");
        printf("%x",(ep_ptr->bLength));
        printf("\r\nType:\t\t");
        printf("%x",(ep_ptr->bDescriptorType));
        printf("\r\nAddress:\t");
        printf("%x",(ep_ptr->bEndpointAddress));
        printf("\r\nAttributes:\t");
        printf("%x",(ep_ptr->bmAttributes));
        printf("\r\nMaxPktSize:\t");
        printf("%x",(ep_ptr->wMaxPacketSize));
        printf("\r\nPoll Intrv:\t");
        printf("%x",(ep_ptr->bInterval));
}

void USBHID::PrintHidDescriptor(const USB_HID_DESCRIPTOR *pDesc) {
        printf("\r\n\r\nHID Descriptor:\r\n");
        printf("bDescLength:\t\t");
        printf("%x",(pDesc->bLength));

        printf("\r\nbDescriptorType:\t");
        printf("%x",(pDesc->bDescriptorType));

        printf("\r\nbcdHID:\t\t\t");
        printf("%x", (pDesc->bcdHID));

        printf("\r\nbCountryCode:\t\t");
        printf("%x",(pDesc->bCountryCode));

        printf("\r\nbNumDescriptors:\t");
        printf("%x",(pDesc->bNumDescriptors));

        printf("\r\nbDescrType:\t\t");
        printf("%x",(pDesc->bDescrType));

        printf("\r\nwDescriptorLength:\t");
        printf("%x",(pDesc->wDescriptorLength));
}
